...
const FeatureLab = {
  ...
  setup(_, { emit }) {
    ...
    const contractSettings = computed(() => makeSettings('classic', '3.1.3 contract test', {
      badgeShowLimit: 3,
    ...
    function toggleAsyncRefresh() {
      ...
      emit('record', `3.1.3 data refresh: ${nextData.length} countries`);
    }
    ...
    async function addCountryFromFilter(query) {
      ...
      emit('record', `3.1.3 async add start: ${requestNumber}: ${query}`);
      ...
      emit('record', `3.1.3 async add resolved: ${requestNumber}: ${query}`);
      return nextItem;
    }

    function updateContractSelection(items) {
      contractSelected.value = items;
      emit('record', `3.1.3 contract change: ${items.length} selected`);
    }
