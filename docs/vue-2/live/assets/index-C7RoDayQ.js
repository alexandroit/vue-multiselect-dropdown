...
inline content inside <textarea> will be ignored when using v-model. Declare initial values in the component's data option instead.`);var r=e.attrsMap.type,i=n||{},a=i.lazy,o=i.number,s=i.trim,c=a||ie&&r===`range`?`change`:`input`,l=!a&&r!==`range`,u=e.tag===`input`||e.tag===`textarea`,d=u?`$event.target.value`+(s?`.trim()`:``):`$event`,f=o||r===`number`?t+`=_n(`+d+`)`:t+`=`+d;if(u&&l&&(f=`if($event.target.composing)return;`+f),r===`file`&&$(`<`+e.tag+` v-model="`+t+`" type="file">:
...
`)}function fe(e,t,n){return JSON.stringify({data:e.dataName,selected:t,settings:n},null,2)}l.default.component(`demo-case`,{components:{VueMultiselectDropdown:ee},props:{test:{type:Object,required:!0}},data(){return{selected:this.test.initialSelected.slice(),localData:this.test.data.slice(),disabled:!1}},computed:{actualSettings(){return Object.assign({},this.test.settings,this.test.allowDisabledToggle?{disabled:this.disabled}:{})},templateSnippet(){return ue(this.test)},jsSnippet(){return de(this.test,this.selected)},jsonSnippet(){return fe(this.test,this.selected,this.actualSettings)},validationMessage(){return this.test.formValidation?this.selected.length?`Valid selection.`:`Select at least one skill.`:``}},methods:{itemLabel:ae,updateSelection(e){this.selected=e,this.record(this.test.eventPrefix+` change`,e)},record(e,t){let n=Array.isArray(t)?`${t.length} items`:ae(t);this.$emit(`record`,`${e}: ${n}`)},appendNextChunk(e){if(!this.test.note)return;let t=j.slice(this.localData.length,this.localData.length+20);t.length?(this.localData=this.localData.concat(t),this.record(this.test.eventPrefix+` lazy`,`${t.length} rows`)):this.record(this.test.eventPrefix+` lazy`,`no more rows`)},addFilterItem(e,t){this.localData=this.localData.concat(t),this.record(this.test.eventPrefix+` created`,e)},renderOption(e,t,n){return n(`span`,{class:`option-row`},[n(`span`,{class:`swatch`,style:{backgroundColor:e.color||`#2563eb`}}),n(`span`,[n(`strong`,[t.label]),n(`small`,[e.detail||e.capital||e.region||``])])])},renderBadge(e,t,n){return n(`span`,{class:`swatch-chip`},[n(`span`,{class:`swatch`,style:{backgroundColor:e.color||`#2563eb`}}),t.label])},openDropdown(){this.$refs.dropdown.openDropdown()},closeDropdown(){this.$refs.dropdown.closeDropdown()},focusSearch(){this.$refs.dropdown.focusSearch()},selectAll(){this.$refs.dropdown.selectAll()},clearSelection(){this.$refs.dropdown.clearSelection()}},template:`
  ...
  `}),new l.default({el:`#app`,components:{VueMultiselectDropdown:ee},data(){return{skins:re,activeSkin:`classic`,events:[],countries:O,skinSwitchSelected:O.slice(1,4),overlayCase:F(`material`,`00`,`overlay-bottom`,`Clipping-safe dropdown with appendToBody`,`overlayCountries`,k,`overlaySelected`,`overlaySettings`,k.slice(0,1),`Dialog dropdown`,{appendToBody:!0,tagToBody:!0,autoPosition:!0,position:`bottom`,maxHeight:180,badgeShowLimit:2},{overflow:!0}),classicCases:le(`classic`),materialCases:le(`material`),extraCases:[F(`dark`,`13`,`dark-basic`,`Dark skin`,`countries`,O,`darkSelected`,`darkSettings`,O.slice(0,3),`Dark dropdown`,{badgeShowLimit:2}),F(`custom`,`14`,`custom-basic`,`Custom skin`,`countries`,O,`customSelected`,`customSettings`,O.slice(2,5),`Custom dropdown`,{badgeShowLimit:2}),F(`brand`,`15`,`brand-basic`,`Brand skin`,`countries`,O,`brandSelected`,`brandSettings`,O.slice(1,5),`Brand dropdown`,{badgeShowLimit:3}),F(`classic`,`16`,`form-validation`,`Controlled form validation`,`skills`,ie,`skillSelected`,`skillSettings`,[],`Select skills`,{groupBy:`category`,badgeShowLimit:3},{formValidation:!0}),F(`classic`,`17`,`methods`,`Instance methods`,`countries`,O,`methodSelected`,`methodSettings`,O.slice(0,2),`Method controlled`,{badgeShowLimit:3},{methods:!0}),F(`classic`,`18`,`add-filter-item`,`Search and add item`,`countries`,O.slice(0,4),`createdSelected`,`createdSettings`,O.slice(0,2),`Add country`,{addNewItemOnFilter:!0,addNewButtonText:`Add country`,badgeShowLimit:3}),F(`classic`,`19`,`disabled-toggle`,`Disabled toggle`,`countries`,O,`disabledToggleSelected`,`disabledToggleSettings`,O.slice(0,2),`Toggle disabled`,{badgeShowLimit:3},{allowDisabledToggle:!0})]}},computed:{skinSwitchSettings(){return ce(this.activeSkin,`Skin `+M(this.activeSkin),{badgeShowLimit:3,maxHeight:220})},skinSwitchTemplate(){return[`<vue-multiselect-dropdown`,`  :data="countries"`,`  v-model="skinSwitchSelected"`,`  :settings="skinSwitchSettings"`,`/>`].join(`
`)},skinSwitchJs(){return[`skinSwitchSelected: ${se(this.skinSwitchSelected)},`,``,`skinSwitchSettings: makeSettings('${this.activeSkin}', 'Skin ${M(this.activeSkin)}', { badgeShowLimit: 3, maxHeight: 220 })`].join(`
`)},skinSwitchJson(){return JSON.stringify({data:`countries`,selected:this.skinSwitchSelected,settings:this.skinSwitchSettings},null,2)}},methods:{labelFor:M,record(e){this.events=[e].concat(this.events).slice(0,10)},updateSkinSwitch(e){this.skinSwitchSelected=e,this.record(`skin switch change: `+e.length+` items`)}},template:`
    <main class="page">
      <header class="topbar">
        <p class="eyebrow">Vue ${ne} runtime</p>
        <h1>@stackline/vue-multiselect-dropdown ${te}</h1>
      </header>
      ...
      <section class="skin-section skin-switcher-section">
        ...
        <article class="example-row custom-skin-sample">
          <div class="demo-cell">
            ...
            <vue-multiselect-dropdown
              :data="countries"
              :value="skinSwitchSelected"
              :settings="skinSwitchSettings"
              @input="updateSkinSwitch"
            />
